...
If we want to automate generating a classification rate for a knn model with particular predictors, we have two major choices: loop through different values of $k$ or write a function and `map()` across different values of $k$.

__Class Exercise 1__. First, we will see how this would work if we were to write a for loop for different values of $k$:
...
__Class Exercise 2__. Next, we will take a functional programming approach, by writing a function and mapping different values of $k$ through that function.
