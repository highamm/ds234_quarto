...
The structure of this section will be a bit different than the structure of the previous sections. We will complete much of this material on handouts that we will fill in by-hand.
:::
...
To introduce knn, we will be using `pokemon_full.csv` data. Pokemon have different Types: we will use `Type` as a categorical response that we are interested in predicting. For simplicity, we will only use Pokemon's primary type and we will only use __4__ different types: 
...
In order to develop our knn model (note that we still haven't discussed what knn actually is yet!), we first need to discuss terms that apply to almost __all__ predictive/classification modeling: training and test data. A __training__ data set is a subset of the full data set used to fit various models. For the example below, the training data set will be just 15 observations for pedagogical purposes. More commonly, the training data set will contain 50%-80% of the observations in the full data set.

A __test__ data set consists of the remaining 20%-50% of the observations not in the training data set. A test data set is used to assess different the performances of various models that were fit using the training data set. 
...
We are using the training vs. test sample method in this class because it's the simplest: if we wanted to take this a step further, we'd repeat the training and test process 5 or 10 times, using what's known as __k-fold cross-validation.__
:::

1. Explain what `anti_join()` joins on when `by` isn't specified and why _not_ specifying a `by` argument works for this example.
...
sqrt((15 - 9) ^ 2 + (350 - 250) ^ 2) ## fire
sqrt((15 - 15) ^ 2 + (350 - 505) ^ 2) ## dark
```
...
```{r, echo = FALSE, results = "hide"}
sqrt((0 - 1) ^ 2 + (0 - 0.39) ^ 2) ## Fire
sqrt((1 - 1) ^ 2 + (1 - 0.39) ^ 2) ## Dark
```
...
In order to examine the performance of a particular model, we'll create a __confusion matrix__ that shows the results of the model's classification on observations in the test data set. An equivalent name for the confusion matrix is a __classification table__.
...
Instead of having 15 Pokemon in our training data set, we now have __70__ pokemon to give a more reasonable number. The test set has the remaining __50__ pokemon.
:::
