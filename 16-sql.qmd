# Introduction to SQL with `dbplyr` {#sql}
...
__Exercise 3__. Obtain the distribution of the `surface` variable by making a table of the total number of matches played on each surface in the 2019 season using `dplyr` functions on `tennis_db`. Then, use `show_query()` to show the corresponding `SQL` code.
...
__Exercise 4__. Examine the `SQL` code below and write equivalent `dplyr` code.
...
__Class Exercise 1__. Examine the `SQL` code below and write equivalent `dplyr` code.
...
__Your Turn 2__. Perform a query of your choosing on `tennis_db` and use the `show_query()` function to show the corresponding `SQL` code.

__Your Turn 3__. Try to run a function from `lubridate` or `forcats` on `tennis_db` with `mutate()`. Does the function work? Did you expect it to work?

__Your Turn 4__. Run the following code and write how the `!` is translated to `SQL`.
...
__Your Turn 5__. Run the following code and write how the `%in%` symbol is translated to `SQL`.
