...
* Watch and follow along with a <a href="https://youtu.be/quN7AQkfKBk" target="_blank">video on installing R</a> (or a <a href="https://youtu.be/LagoBJXjsTk" target="_blank">captioned version</a>). 
    * starting link: <a href="https://www.r-project.org/" target="_blank">https://www.r-project.org/</a>. 

* Watch and follow along with a <a href="https://youtu.be/tKoPy_FZh9w" target="_blank">video on installing R Studio</a> (or a <a href="https://youtu.be/p1gtbCRaJ4c" target="_blank">captioned version</a>).
    * starting link: <a href="https://posit.co/products/open-source/rstudio" target="_blank">https://posit.co/products/open-source/rstudio</a>. Note that this webpage has changed since the posted video was made. But, this step should be somewhat straightforward on the new webpage: scroll down a little and click "Download R Studio", then click either the `"Windows 11"` link if you have Windows or the `"macOS 14+"` link if you have a Mac! 
    
* Watch and follow along with a <a href="https://youtu.be/RTbHwpOTxsc" target="_blank">video on installing R packages and making an R project </a> (or a <a href="https://youtu.be/bnohB_3xVH0" target="_blank">captioned version</a>). 
...
## Embedding Resources {#chapexercise-1}

When you open a new `.qmd` file with (File -> New File -> `Quarto Document` -> OK), you should see at the top of the file what is called a `YAML` header. The `YAML` header starts with three dashes `---` and ends with three dashes `---`. Yours should look something like:
...
By default, when you render the file, Quarto puts all rendered figures and tables into a separate folder on your computer and references these figures and tables using file paths. This is not convenient for submitting things to Canvas: when you submit the `.html` file will point to locations that do not exist since Canvas will not have this extra folder with all of the figures and tables!

So, we change our YAML header to:
...
The `embed-resources: true` says to embed the tables and figures directly into the rendered `.html` file. Therefore, with the figures and tables directly embedded, the files you submit to Canvas will contain your figures. 

We will talk more about YAML headers in a few weeks. For now, the YAML header above is something you should copy and paste into every file that you intend to submit.
