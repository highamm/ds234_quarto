...
__Exercise 3__. There are a lot of named colours that you can use in `R`. After looking through some at <a href="https://www.stat.auckland.ac.nz/~ihaka/downloads/R-colours-a4.pdf" target="_blank">https://www.stat.auckland.ac.nz/~ihaka/downloads/R-colours-a4.pdf</a>, change the `colour` and `fill` of the histogram again to be colours of your choice.
...
__Class Exercise 2__*. In the fitness data set, what is the relationship between distance and active calories?

<!-- After adding the smoother, investigate the following arguments: (1) `method`, (2) `se`, and (3) `span`. -->


__Class Exercise 3__*. In the fitness data set, what is the relationship between number of steps taken and date? Is there an increasing trend or a decreasing trend or neither? 
...
__Class Exercise 4__*. In the stat-survey data set, what is the relationship between GPA and Exercise? Is that relationship different depending on sex? depending on sport? depending on sex-sport combination?
...
__Your Turn 6__. On your own, construct a plot of your choosing, with any variables from either of the data sets we have used so far! Give a one sentence interpretation of your findings from your plot.

__Your Turn 7__. A common theme that we'll see throughout the course is that it's advantageous to know as much background information as possible about the data set we are analyzing. Data sets will be easier to analyze and pose questions about if you're familiar with the subject matter. 
...
```{r}
ggplot(data = fitness_full) +
  geom_point(aes(x = Date, y = steps, colour = season)) +
  geom_smooth(aes(x = Date, y = steps, colour = season))
```
