...
#| echo: false
#| output: false
prices_tidy <- prices_df |> pivot_longer(2:7, names_to = "temp", values_to = "price") |>
...
ggplot(data = prices_rice, aes(x = year, y = price)) +
  geom_line(aes(group = city), alpha = 0.4)
```
...
You'll need to add something to your `pivot_longer()` function to convert the variable `name_of_your_year_variable` to numeric. We haven't talked about variable types yet so, but you will need to change the variable type of your year variable to numeric. After pivoting, pipe into a `|> mutate(name_of_your_year_variable = as.numeric(name_of_your_year_variable))` to change your year variable type to numeric.
:::
...
baseball_tidy <- name_of_your_baseball_data |>
...
Run the code above so that the parsing is saved to `baseball_tidy`.
...
__Your Turn 10__. Sometimes for graphs involving cost or salary, we want to take into account the inflation rate. Google what the inflation rate was between 2011 and 2018 (Google something like "inflation rate from 2011 to 2018" and you should be able to find something). Adjust all of the 2011 salaries for inflation so that they are comparable to the 2018 salaries. 

__Your Turn 11__. Construct a graph that shows the salary depreciation from top paid players in each position to lesser paid players in each position for the year 2018 only. Which position has the largest depreciation? 
