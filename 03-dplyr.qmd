...
__Class Exercise 2__. Create a table that shows how many times each name appears as the most popular name of the sex for the year. That is, if `"Emma"` was the most popular female name in 1995, 1997, and 2001, then it would get a value of `"3"` in the summary table. To do so, we will first make some notes about what `group_by()` does with more than 1 variable and what `ungroup()` does. In this exercise, we will use the `rank()` function as well.

__Class Exercise 3__. Construct the same table as in class exercise 4, but figure out a way to construct the table __without__ using the `rank()` function.
