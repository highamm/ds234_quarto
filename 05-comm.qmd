...
```{r, echo = FALSE}
## When using Quarto, all code must run for the document to render. 
## Therefore, someone else can (usually) run your code. Anyone can
## see exactly what was done in your analysis (if there were outliers 
## removed, what data wrangling steps took place, etc.). This is not
## necessarily the case with R scripts and Word.
```

3. Why is an `Quarto` analysis easier to make more reproducible than an analysis with Excel?

```{r, echo = FALSE}
## Simiilar to Number 2. In Excel, it's often hard to see what was done
## with the data. You don't see if outliers were taken out. Quarto
## makes both of these steps explicit because code is included within
## the document itself.
```
...
```{r, echo = FALSE}
## the removal of outliers: it's not clear in Excel because once he deletes
## the rows, they are gone forever. In Quarto, filter() makes it easy
## for us to see which rows were removed.

##Chaz changes the limits of the boxplot y-axis. If I were to try to reproduce
##it, I might have a hard time figuring out exactly how he changed these limits

## Did Chaz do a two-sample, equal variance t-test? Unequal variance?
## One-sided test? Two-sided test? It's hard to tell here, but the R code
## would make it clear.
```
...
```{r, echo = FALSE}
## solution not given: will depend on the table you chose to make
...
```{r, echo = FALSE}
## to set a global option, change what's inside the set-up chunk to read:
## (get rid of the commented hashtags)

## ```{r setup, include=FALSE}
## knitr::opts_chunk$set(fig.height = 3)
## ```

## for the local options change the current chunk to:
## (get rid of the commented hashtags)
...
```{r, echo = FALSE}
## in annotate(), you have to specify specific x and y coordinates of your
## annotation. This does allow more fine tuning than geom_label(), but
## becomes more of a pain when there are more labels you need to make. You
## need a different annotate() statement for each label you want to draw on
## the plot.
```
