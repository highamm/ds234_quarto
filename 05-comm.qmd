...
Because the data set is loaded every time `R` is started up, there is no need to have a line that reads in the data set. We can glimpse at the data with
...
Explore the data set with `glimpse(country_df)` and `?gapminder` before proceeding to the following exercises.

__Class Exercise 1__. Construct a plot that lets us explore the relationship between life expectancy and year for the different countries in the data set.

__Class Exercise 2__. What can we do to make our plot more readable? Make some changes here, based on what we discussed in this section. 
...
__Class Exercise 6__. If there are a lot of overlapping points or overlapping lines, we can use `alpha` to control the transparency of the lines. Change `alpha` so that the lines are more transparent and the overall plot is more readable.
...
* one of your code chunks gets hidden.

* the warnings/messages that `R` prints by default are hidden in all code chunks.

* the default figure height is 4 instead of the default 7.
...
__Your Turn 2__. Go back to the line plot that we made with the fitness data set of `steps` vs. `Start` date. Add a label to the peak at April 14, 2019 that says `"half-marathon"` and a label to the peak at May 24, 2021 that says `"grand canyon"`.
