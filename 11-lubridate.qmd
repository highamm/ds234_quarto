...
Once an object is in the `<date>` format, there are some special functions in `lubridate` that can be used on that date variable. To investigate some of these functions, we will pull stock market data using the `tidyquant` package. Install the package, and run the following code, which gets stock market price data on Apple, Nintendo, Chipotle, and the S & P 500 Index from 2015 to now. 

```{r}
library(tidyquant)
stocks_long <- tq_get(c("AAPL", "CMG", "NTDOY", "SPY"), get = "stock.prices")
```
...
* `date`, the opening date for the stock market.
* `symbol`, a factor with 4 levels, one for each stock.
...
But, there's other information that we can get from the `date` variable. We might be interested in things like day of the week, monthly trends, or yearly trends. To extract variables like "weekday" and "month" from a `<date>` variable, there are a series of functions that are fairly straightforward to use. We will discuss the `year()` `month()`, `mday()`, `yday()`, and `wday()` functions.
...
stocks_long |> mutate(year_stock = year(date))
stocks_long |> mutate(month_stock = month(date))
stocks_long |> mutate(day_stock = mday(date))
```
...
```{r, appendix = TRUE}
stocks_long |> mutate(day_in_year = yday(date))
```
...
```{r, results = "hide", appendix = TRUE}
stocks_long |> mutate(day_of_week = wday(date))
stocks_long |> mutate(day_of_week = wday(date,
...
__Class Exercise 4__. Suppose that you want to explore how often a stock's closing price was higher on Day xxx than it was on closing on the previous Day xxx minus 1. How might you answer this question? What if you wanted to compare week-to-week instead of day-to-day? What if you wanted to compare month-to-month instead of day-to-day?
...
__Your Turn 2__. Visualize the popularity of your two variables over time.
...
__Your Turn 3__. Using your data set that explored a variable or two from 2004 through now, make a table of the __maximum__ popularity and the __average__ popularity for each year.
