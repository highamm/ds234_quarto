...
1. An exercise set due on Monday (worth __3__ points), a take-home quiz due on Wednesday (worth __7__ points), and a handwritten in-class quiz on Wednesday (worth __70__ points). We should have 4 total modules follow this structure.

2. An exercise set due on Monday (worth __3__ points), a take-home quiz due on Wednesday (worth __7__ points), and an in-class coding quiz taken on a computer on Wednesday (worth __70__ points). We should have 4 total modules follow this structure.

3. An exercise set due on Monday (worth __3__ points), a participation assessment (worth __7__ points), a Project due Wednesday (worth __35__ points), and a handwritten in-class assessment specifically about the Project on Wednesday (worth __35__ points). We should have 3 total modules follow this structure.

The lowest module will be dropped from your grade so the total number of points available from all modules is `11 * 80 = 880` points.
...
There is an optional Final Exam worth 40 points. You **must** be on campus for our final exam time if you would like to take the optional final exam. If you do not take the optional final, then the __average__ of your 11 highest module scores will be used for the 40 points for this exam. 

If you take the final exam and if your final exam score percentage is better than your 2nd lowest module grade, then that grade will be replaced with your final exam score. For example, suppose your 12 module scores are: `80, 80, 75, 75, 70, 70, 59, 58, 50, 34, 15, 0`. You take the final exam and score a `36 / 40 = 90%`. Then, the `36` is used for your final exam score and your new module scores would be: `80, 80, 75, 75, 70, 70, 59, 58, 50, 34, 72, 0`. (The second lowest value of `15` is replaced by 90% of 80, which is `72`. The `0` would still be dropped as your lowest module score).

### Breakdown

-   880 points for Modules
-   40 points for the (optional) Final Exam
...
| Week | Date    | Topics                                                      |
|--------------|--------------|-------------------------------------------------|
| 0    | 8/27    | Introduction to `R`, `R Studio`                             |
| 1    | 9/1    | Graphics with `ggplot2`                                     |
| 2    | 9/8     | Data Wrangling  with `dplyr`    
   |
| 3    | 9/15    | Communication with `Quarto` and `ggplot2`                   |
| 4    | 9/22    | Soft Skills and Workflow                                    |
| 5    | 9/29  | Data Tidying with `tidyr`                                   |
| 6    | 10/6    | Base `R`                                                    |
| 7    | 10/13   | Factors with `forcats` and Data Ethics                      |
| 8    | 10/20 | Data Import with `readr`, `jsonlite`, `rvest`, and `tibble` |
| 9    | 10/27   | Data Merging with `dplyr`                                   |
| 10   | 11/3 | Intro to Statistical/Machine Learning with knn                              |
| 11   | 11/10    | Text Data with `tidytext` and `stringr`                     |
| 12   | 11/17   |  Dates and Times with `lubridate`  |
| 13   | 12/1   | Connections to STAT and CS                                  |
| 14   | 12/8    | Databases and SQL with `dbplyr`                             |
