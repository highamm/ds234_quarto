...
Why are `births` missing in the first few rows of `combined_left` but not in `combined_left |> tail()`?
...
__Class Exercise 0__. Examine the following data sets (the first is `df1` and the second is `df2`) and then, without running any code, answer the following questions.
...
__Class Exercise 3__. The remaining class exercises use three data frames containing information on television shows from the Internet Movie DataBase (IMDB):

* `imdb_episodes` contains an episode identification number, a show identification number, the average episode rating according to IMDB voters, the number of votes and the name of the episode.

```{r}
imdb_episodes <- read_csv(here::here("data/imdb_episodes.csv"))
imdb_info <- read_csv(here::here("data/imdb_info.csv"))
imdb_showrating <- read_csv(here::here("data/imdb_showrating.csv"))
```

In this example, identify the __key__.

<!-- The key is the parentTconst. -->

__Class Exercise 4__. Suppose that you want to combine the `imdb_info` and `imdb_showrating` data sets into one data set. Should you use `bind_rows()` or `bind_cols()`? And, for the function that you select, what are you assuming? What might be better to use instead? Create this new data frame and call it `show_info`.

<!-- `bind_cols()`, but I am assuming that the shows appear in the same order in each data set. This might be the case, but a safer option would be to use a join. -->

```{r}
show_info <- inner_join(imdb_info, imdb_showrating, join_by(parentTconst == parentTconst))
```

__Class Exercise 5__. If you were to use a `left_join()` on the `imdb_episodes` data with the `show_info` data from the previous exercise, what might the new data set be getting rid of, in the context of this example?

<!-- we could be getting rid of any show in the show info data that does not have any rated episodes in imdb episodes. -->

__Class Exercise 6__. If you were to use a `left_join()` on the `show_info` data from the previous exercise with the `imdb_episodes` data, what might the new data set be getting rid of, in the context of this example?

<!-- we could be getting rid of any episodes in imdb_episodes that do not have show information in show info. -->

__Class Exercise 7__. Using a join function, keep only the shows in `show_info` that have more than 10 episodes in the `imdb_episodes` data set.

```{r}
#| echo: false
#| output: false
imdb_10 <- imdb_episodes |> group_by(show_id) |>
  summarise(n_episodes = n()) |>
  filter(n_episodes > 10)
semi_join(show_info, imdb_10, join_by(parentTconst == show_id))
```

__Class Exercise 8__. Using a join function, keep only the episodes in `imdb_episodes` where the show itself has an average rating at 8 or above and has at least 200000 show votes.

```{r}
#| echo: false
#| output: false
show_popular <- show_info |> filter(averageRating >= 8.0 & numVotes >= 200000)
semi_join(imdb_episodes, show_popular, join_by(show_id == parentTconst))
```
