...
The `mtcarsex.csv` has observations on different car models with variables that include things like gas mileage, number of cylinders, etc. Read in the `mtcarsex.csv` data set with the following code. Then, examine the data set.
...
Go to the following website to look at the table of data that we want to scrape: <a href="https://saintsathletics.com/sports/baseball/stats/2025" target="_blank">https://saintsathletics.com/sports/baseball/stats/2025</a>.
...
There's now over 50 different tables! See if you can figure out where the first few tables are coming from on the website.
...
__Class Exercise 4__. Construct a barplot of highest ranked ATP (men's tennis) players right now. You will need to scrape some data from <https://en.wikipedia.org/wiki/Current_tennis_rankings> to construct this plot. 
...
tabs <- read_html(url) |> html_nodes("table")
tabs[[1]] |> html_table(header = FALSE)

atp_df <- tabs[[1]] |> html_table(header = FALSE) |>
  slice(-(1:2)) |>
  rename(player = X2, 
         points = X3,
         move = X4,
